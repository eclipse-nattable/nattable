...
public class DefaultGlazedListsFilterStrategy<T> implements IFilterStrategy<T> {
    ...
    private static final Log log = LogFactory
            .getLog(DefaultGlazedListsFilterStrategy.class);
     ...
     *            configurations.
     */
    public DefaultGlazedListsFilterStrategy(FilterList<T> filterList,
            IColumnAccessor<T> columnAccessor, IConfigRegistry configRegistry) {
        this(filterList, new CompositeMatcherEditor<T>(), columnAccessor,
                configRegistry);
    ...
    public void applyFilter(Map<Integer, Object> filterIndexToObjectMap) {
        ...
        try {
            ...
            for (Entry<Integer, Object> mapEntry : filterIndexToObjectMap
                    .entrySet()) {
                Integer columnIndex = mapEntry.getKey();
                String filterText = getStringFromColumnObject(columnIndex,
                        mapEntry.getValue());
                ...
                if (stringMatcherEditors.size() > 0) {
                    CompositeMatcherEditor<T> stringCompositeMatcherEditor = new CompositeMatcherEditor<T>(
                            stringMatcherEditors);
                    stringCompositeMatcherEditor
                            .setMode(CompositeMatcherEditor.OR);
    ...
    protected ThresholdMatcherEditor<T, Object> getThresholdMatcherEditor(
            Integer columnIndex, Object threshold,
            Comparator<Object> comparator,
            Function<T, Object> columnValueProvider, MatchType matchOperation) {
        ThresholdMatcherEditor<T, Object> thresholdMatcherEditor = new ThresholdMatcherEditor<T, Object>(
                threshold, null, comparator, columnValueProvider);

        FilterRowUtils
                .setMatchOperation(thresholdMatcherEditor, matchOperation);
        return thresholdMatcherEditor;
    }

    /**
     * @return Function which exposes the content of the given column index from
     *         the row object
     */
    protected FunctionList.Function<T, Object> getColumnValueProvider(
     ...
     */
    protected TextMatcherEditor<T> getTextMatcherEditor(Integer columnIndex,
            TextMatchingMode textMatchingMode, IDisplayConverter converter,
            String filterText) {
        TextMatcherEditor<T> textMatcherEditor = new TextMatcherEditor<T>(
                getTextFilterator(columnIndex, converter));
        textMatcherEditor.setFilterText(new String[] { filterText });
        textMatcherEditor
                .setMode(getGlazedListsTextMatcherEditorMode(textMatchingMode));
    ...
    protected TextFilterator<T> getTextFilterator(final Integer columnIndex,
            final IDisplayConverter converter) {
        return new TextFilterator<T>() {
            @Override
            public void getFilterStrings(List<String> objectAsListOfStrings,
                    T rowObject) {
                Object cellData = columnAccessor.getDataValue(rowObject,
                        columnIndex);
                Object displayValue = converter
                        .canonicalToDisplayValue(cellData);
     ...
     */
    public int getGlazedListsTextMatcherEditorMode(
    ...
    /**
     * Returns the CompositeMatcherEditor that is created and used by this
     * IFilterStrategy. In prior versions it was necessary to create the
     * CompositeMatcherEditor outside this class and use it as constructor
     * parameter. We changed this to hide that implementation from users and to
     * ensure that filter operations and possible listeners are executed thread
     * safe. Otherwise there might be concurrency issues while filtering.
     * <p>
     * If you want to use additional filtering you should now use this method to
     * work on the created CompositeMatcherEditor instead of creating one
     * outside. For static filtering additional to the filter row you might want
     * to consider using the DefaultGlazedListsStaticFilterStrategy.
     * 
     * @return The CompositeMatcherEditor that is created and used by this
     *         IFilterStrategy.
     */
