...
public class PreserveSelectionModelTest {
	...
	/**
	 * Each row consist of an array of String, one String for each column, i e each cell is a
	 * String.
	 */
	...
	public void setUp() throws Exception {
		...
		/*
		 * row 0 in the immediate table is row 4 in the underlying virtual table coordinate
		 * system:
		 *
		 * row 0: A B C
		 *
		...
		testee = new PreserveSelectionModel<String[]>(selectionLayer, rowDataProvider, rowIdAccessor);
	}

	/**
	 * row 0 in the immediate table is row 4 in the underlying virtual table coordinate system:
	 *
	 * row 0: A B C
	 *
	 * row 1: _ D E
	...
	private boolean isCellSelected(int columnPosition, int rowPosition) {
		boolean singularVersion = testee.isCellPositionSelected(columnPosition, rowPosition);
	...
	public void getSelections_Ignores_Invisible_Rows() {
		...
		assertEquals(new Rectangle(1, 0, 1, 1), testee.getSelections().iterator().next());
	}
	...
	public void getSelectedColumnPositions() {
		...
		assertEquals(Arrays.toString(new int[] { 0, 1, 2 }), Arrays.toString(selectedColumns));
	}
	...
	@Test
	public void isColumnFullySelected_Copes_With_Overlapping_Regions() throws Exception {
		testee.addSelection(new Rectangle(1, 0, 2, 2));
	...
	@Test
	public void isColumnFullySelected_Copes_With_Combining_Adjacent_Individually_Selected_Cells() throws Exception {
		testee.addSelection(1, 0);
		testee.addSelection(1, 1);
	...
	public void getFullySelectedColumnPositions() {
		...
		assertEquals(Arrays.toString(new int[] { 1, 2 }), Arrays.toString(fullySelectedColumns));

		fullySelectedColumns = testee.getFullySelectedColumnPositions(1);
		assertEquals(Arrays.toString(new int[] { 0, 1, 2 }), Arrays.toString(fullySelectedColumns));

		fullySelectedColumns = testee.getFullySelectedColumnPositions(3);
		assertEquals(Arrays.toString(new int[] { 2 }), Arrays.toString(fullySelectedColumns));
	}
	...
	public void getSelectedRowPositions() {
		...
		HashSet<Range> actualSelectedRowPositions = new HashSet<Range>(testee.getSelectedRowPositions());
	...
	@Test
	public void isRowPositionFullySelected_Copes_With_Overlapping_Regions() throws Exception {
		testee.addSelection(new Rectangle(1, 0, 2, 2));
	...
	@Test
	public void isRowPositionFullySelected_Copes_With_Combining_Adjacent_Individually_Selected_Cells() throws Exception {
		testee.addSelection(1, 0);
		testee.addSelection(2, 0);
	...
	public void getFullySelectedRowPositions() {
		...
		assertEquals(Arrays.toString(new int[] { 0, 1 }), Arrays.toString(fullySelectedRows));

		fullySelectedRows = testee.getFullySelectedRowPositions(1);
		assertEquals(Arrays.toString(new int[] { 0, 1, 2 }), Arrays.toString(fullySelectedRows));

		fullySelectedRows = testee.getFullySelectedRowPositions(3);
		assertEquals(Arrays.toString(new int[] { 0 }), Arrays.toString(fullySelectedRows));
	}
	...
	@Test
	public void getLastSelectedCell_Copes_With_Missing_Marker() throws Exception {
		assertEquals(SelectionLayer.NO_SELECTION, testee.getLastSelectedCell().x);
		assertEquals(SelectionLayer.NO_SELECTION, testee.getLastSelectedCell().y);
	}
	...
	@Test
	public void getLastSelectedRegion_Copes_With_Missing_Marker() throws Exception {
	...
	@Test
	public void setLastSelectedRegion_On_Parameters_Copys_Data() throws Exception {
		Rectangle oldReference = new Rectangle(0, 0, 1, 1);
		testee.lastSelectedRegion = oldReference;
