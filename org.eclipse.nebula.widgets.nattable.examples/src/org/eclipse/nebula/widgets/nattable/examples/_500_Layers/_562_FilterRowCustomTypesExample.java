...
public class _562_FilterRowCustomTypesExample extends AbstractNatExample {
	...
	public Control createExampleControl(Composite parent) {
		...
		EventList<MyRowObject> eventList = GlazedLists.eventList(createMyRowObjects(50));
		TransformedList<MyRowObject, MyRowObject> rowObjectsGlazedList = GlazedLists.threadSafeList(eventList);
		
		//use the SortedList constructor with 'null' for the Comparator because the Comparator
		//will be set by configuration
		SortedList<MyRowObject> sortedList = new SortedList<MyRowObject>(rowObjectsGlazedList, null);
		// wrap the SortedList with the FilterList
		FilterList<MyRowObject>filterList = new FilterList<MyRowObject>(sortedList);
		
		IDataProvider bodyDataProvider = 
			new ListDataProvider<MyRowObject>(filterList, columnPropertyAccessor);
		DataLayer bodyDataLayer = new DataLayer(bodyDataProvider);
		
		bodyDataLayer.setConfigLabelAccumulator(new ColumnLabelAccumulator());
		
		//layer for event handling of GlazedLists and PropertyChanges
		GlazedListsEventLayer<MyRowObject> glazedListsEventLayer = 
			new GlazedListsEventLayer<MyRowObject>(bodyDataLayer, filterList);
