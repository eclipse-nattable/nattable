...
public class ImagePainter extends BackgroundPainter {
    ...
    public ICellPainter getCellPainterAt(int x, int y, ILayerCell cell, GC gc,
        ...
        if (image != null) {
            ...
            int x0 = bounds.x
                    + CellStyleUtil.getHorizontalAlignmentPadding(cellStyle,
                            bounds, imageBounds.width);
            int y0 = bounds.y
                    + CellStyleUtil.getVerticalAlignmentPadding(cellStyle,
                            bounds, imageBounds.height);
            if (x >= x0 && x < x0 + imageBounds.width && y >= y0
                    && y < y0 + imageBounds.height) {
                return super.getCellPainterAt(x, y, cell, gc, bounds,
                        configRegistry);
            }
        }
        return null;
    ...
    protected Image getImage(ILayerCell cell, IConfigRegistry configRegistry) {
        return image != null ? image : CellStyleUtil.getCellStyle(cell,
