...
/**
 * Theme configuration that overrides the stylings set in the {@link DefaultNatTableThemeConfiguration}
 * to give the NatTable a more modern look&feel.
 * 
 * <p>
 * This default theme configuration does not set a different value for the filter row cell painter,
 * as in this case it would override the specialized painter that needs to be registered for combobox
 * filterrows.
 * </p>
 * @author Dirk Fauth
 *
...
public class ModernNatTableThemeConfiguration extends DefaultNatTableThemeConfiguration {
	{
		...
		this.sortHeaderCellPainter = new BackgroundPainter(
				new PaddingDecorator(
						new SortableHeaderTextPainter(
								new TextPainter(false, false), CellEdgeEnum.RIGHT, new SortIconPainter(false), false, 0, false), 
						0, 2, 0, 5, false));
		...
		TreeImagePainter treeSelectionImagePainter = 
				new TreeImagePainter(false, GUIHelper.getImage("right_inv"), GUIHelper.getImage("right_down_inv"), null);  //$NON-NLS-1$//$NON-NLS-2$
		this.treeStructureSelectionPainter = new BackgroundPainter(new PaddingDecorator(
				new IndentedTreeImagePainter(10, null, CellEdgeEnum.LEFT, treeSelectionImagePainter, false, 2, true), 0, 5, 0, 5, false));
