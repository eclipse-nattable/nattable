...
public class StructuralChangeEventHelper {
	...
	/**
	 * Will check for events that indicate that rows has been deleted. In that case the given
	...
	/**
	 * Will check for events that indicate that rows are added. In that case the given
	...
	/**
	 * Will check for events that indicate that columns has been deleted. In that case the given
	 * cached indexes for the given layer need to be updated because the index of the columns might have changed.
	 * E.g. Column with index 3 is hidden in the given layer, deleting column at index 1 will cause the column 
	 * at index 3 to be moved at index 2. Without transforming the index regarding the delete event, the wrong
	 * column would be hidden.
	 * @param columnDiffs The collection of {@link StructuralDiff}s to handle
	 * @param underlyingLayer The underlying layer of the layer who caches the indexes. Needed to
	 * 			translate the transported column positions to indexes, because the conversion to the
	 * 			layer who caches the index is done before it is fired further in the layer stack
	 * @param cachedColumnIndexes The collection of indexes that is cached by the layer that needs
	 * 			transformation
	 * @param handleNotFound flag to tell whether the not found column indexes should be taken into account
	 * 			or not. Needed for last column checks
	 */
	public static void handleColumnDelete(Collection<StructuralDiff> columnDiffs, ILayer underlyingLayer, 
			Collection<Integer> cachedColumnIndexes, boolean handleNotFound) {
		
		//the number of all deleted columns that don't have a corresponding index anymore (last column cases)
		int numberOfNoIndex = 0;
		List<Integer> toRemove = new ArrayList<Integer>();
		for (Iterator<StructuralDiff> diffIterator = columnDiffs.iterator(); diffIterator.hasNext();) {
	...
	/**
	 * Will check for events that indicate that columns are added. In that case the given
	 * cached indexes need to be updated because the index of the columns might have changed.
	 * E.g. Column with index 3 is hidden in the given layer, adding a column at index 1 will cause the 
	 * column at index 3 to be moved to index 4. Without transforming the index regarding the add event, 
	 * the wrong column would be hidden.
	 * @param columnDiffs The collection of {@link StructuralDiff}s to handle
	 * @param underlyingLayer The underlying layer of the layer who caches the indexes. Needed to
	 * 			translate the transported column positions to indexes, because the conversion to the
	 * 			layer who caches the index is done before it is fired further in the layer stack
	 ...
	 * 			are applied for a specific state (e.g. column hide state)
	 */
	public static void handleColumnInsert(Collection<StructuralDiff> columnDiffs, ILayer underlyingLayer, 
	 ...
	 * This is necessary because reordering itself contains out of two diffs, one for deleting
	 * columns/rows, one for adding them at another position. As these diffs are completely
	 * separated from each other, but the handling does have impact, this check is added. Also
	 * on reordering there is no need for special handling of diffs in the layers that call
	 * this method.
	 * <p>
	 ...
	 * <li>on handling the deletion of index 0 the hidden index 2 would be reduced to 1 (which is wrong because
	 * 		it is reordered not really deleted)</li>
	 * <li>adding the column at index 4 would undo the hidden index modification, because the insertion is
	 * 		handled separately and does not know about the former deletion</li>
	 * </ul>
	 * @param structuralDiffs The collection of StructuralDiffs to check for reordering
	 * @return <code>true</code> if the diff indicates a reordering happened, <code>false</code> if
	 * 			if was not a reordering based on the explanation above.
	 */
