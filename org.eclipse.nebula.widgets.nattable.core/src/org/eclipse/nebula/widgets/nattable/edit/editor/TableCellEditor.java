 ...
 * In combination with the TableCellPainter it can be used to simulate a table within
 * a cell to support editing of collections or arrays of values of an entity.
 * This implementation is a workaround for the missing feature of nested tables in 
 * NatTable 1.x architecture. It is limited to one column editing. If a more complex
 * nested table structure is needed, you need to extend or implement a new table cell editor.
 * <p>
 * Note: This editor is different to other default editors in NatTable in several facts:
 * <ul>
 * <li>The data value in the NatTable cell needs to be an array or a collection of values</li>
 * <li><code>getCanonicalValue()</code> will directly update the underlying NatTable data model</li>
 * <li>Committing the value in the NatTable framework code will simply replace the list reference with itself</li>
 * <li>It does only support validation error styling for conversion and validation errors aswell</li>
 * </ul>
 * 
...
public class TableCellEditor extends AbstractCellEditor {
	...
	/**
	 * Flag to configure whether the adjacent editor on selection movements should always
	 * open the cell for editing instead of moving into the selection state of the table control.
	 */
	private final boolean alwaysOpenEditor;
	/**
	 * The height of the sub cells to use. Setting a value >= 0 will result in using the specified fixed 
	 * sub cell heights, a negative value will result in using the OS default height based on the font.
	 * <p>
	 * Because of limitations in the native table control for some OS, it is not possible to specify
	 * different row heights.
	 */
	private int fixedSubCellHeight;
	/**
	 * Internal focus listener to handle committing and closing of this editor if the focus is lost out
	 * of the editor control AND the editing support editor control.
	 */
	private FocusListener focusListener = new InternalFocusListener();
	/**
	 * Internal ColumnLabelProvider that is used to apply styling to the table viewer aswell as for
	 * the editor controls in the table viewer.
	 */
	private InternalLabelProvider labelProvider;
	/**
	 * Internal ICellEditorValidator that checks if a value is valid based on conversion and
	 * validation rules applied via NatTable IDisplayConverter and IDataValidator
	 */
	protected ICellEditorValidator cellEditorValidator = new ICellEditorValidator() {
		@Override
		public String isValid(Object value) {
			//add validator to check conversion and validation configured in NatTable
			if (displayConverter != null) {
				Object cValue = null;
				try {
					//check if the information can be converted to the correct type
	...
	/**
	 * Creates a TableCellEditor with a default sub cell height of 20 (which is the same as the default
	 * for the TableCellPainter), that moves the selection on committing the value with enter and always
	 * opens the editor of the cell that is currently selected.
	 */
	 ...
	 * @param fixedSubCellHeight The height of the sub cells to use. Setting a value >= 0 will result in 
	 * 			using the specified fixed sub cell heights, a negative value will result in using the OS 
	 * 			default height based on the font. Note that because of limitations in the native table control 
	 * 			for some OS, it is not possible to specify different row heights.
	 * @param moveSelectionOnEnter configure whether the selection should move after a value was
	 * 			committed by pressing enter.
	 * @param alwaysOpenEditor configure whether the adjacent editor on selection movements should always
	 * 			open the cell for editing instead of moving into the selection state of the table control.
	 */
	...
	public Object getCanonicalValue() {
		...
		if (layerCell.getDataValue().getClass().isArray()) {
			...
			Collection cellDataCollection = (Collection)layerCell.getDataValue();
	...
	public void setCanonicalValue(Object canonicalValue) {
		...
		if (values != null) {
			//transform the array of canonical data values to an array of display values
	...
	public Table createEditorControl(Composite parent) {
		//need to directly set the member variable because a TableViewer is not a Control
		//and therefore we can not return the TableViewer here
		viewer = new TableViewer(parent, SWT.FULL_SELECTION);
		viewer.setContentProvider(ArrayContentProvider.getInstance());
		
		//this column is simply added because of the restriction that the first column
		...
		singleColumn.getColumn().setAlignment(HorizontalAlignmentEnum.getSWTStyle(cellStyle));
		...
		tableControl.addListener(SWT.MeasureItem, new Listener() {
			...
			public void handleEvent(Event event) {
				event.height = fixedSubCellHeight + 1; //+1 because of the grid lines
			}
		});
		...
		tableControl.addListener(SWT.Resize, new Listener() {
			...
			@Override
			public void handleEvent(Event event) {
				//it is the second column, because the first column is not visible as we want to
				//support alignment in the cells
				tableControl.getColumn(1).setWidth(tableControl.getClientArea().width);
				
				//set focus on the table viewer and select the cell where the mouse is positioned
				//this can only be done when the control is resized, as the bounds are set in
				...
				if (tableControl.getItemCount() > 0)
					viewer.editElement(((StructuredSelection)viewer.getSelection()).getFirstElement(), 1);
			}
		});
	 ...
	 * the data object is not a Collection or Array.
	 * @param cellData The cellData that should be checked for its type.
	 * @return The Object[] representation of the data object if it is of type
	 * 			Collection or Array, or <code>null</code> if the data object
	...
	/**
	 * Note that because of limitations to native tables of the OS, it is not possible to specify
	 * different row heights.
	 * @return The height of the sub cells to use. A value >= 0 results in using the specified fixed 
	 * 			sub cell heights, a negative value results in using the OS default height based on
	 * 			the font.
	 */
	 ...
	 * Setting the value to a negative number will result in using the OS default height based on
	 * the font.
	 * <p>
	 * Note that because of limitations to native tables of the OS, it is not possible to specify
	 * different row heights.
	 * @param fixedSubCellHeight The height of the sub cells to use.
	 */
	 ...
	 * {@inheritDoc}
	 * <p>
	 * Note: We need to override this to not register the FocusListener.
	 * 		 This is necessary because on editing the TableViewer, a Text control will be
	 * 		 created that gains focus. This would mean to close the editor and break the
	 * 		 whole use case.
	 */
	@Override
	 ...
	 * If we only work with the values themselves and the collection contains the
	 * same values like for example the same Strings, calling editElement() will
	 * always jump to the first element in the table with that value instead of 
	 * the selected one.
	 * With this wrapper we ensure that the selected value is edited because we
	 ...
	 * Will use a org.eclipse.jface.viewers.TextCellEditor for editing.
	 */
	protected class TableCellEditingSupport extends EditingSupport {
		...
		public TableCellEditingSupport() {
			super(viewer);
			editor = new org.eclipse.jface.viewers.TextCellEditor(viewer.getTable()); 
		}
			...
			editor.getControl().addTraverseListener(new TraverseListener() {
				...
				public void keyTraversed(TraverseEvent event) {
					if (event.keyCode == SWT.TAB) {
						TableCellEditingSupport.this.setValue(element, ((Text)editor.getControl()).getText());
				...
				public void keyPressed(KeyEvent event) {
					if (event.keyCode == SWT.CR || event.keyCode == SWT.KEYPAD_CR) {
						...
						if (editor.isValueValid()) {
							...
							//if move selection and not last item -> move the selection one down
							if (moveSelectionOnEnter && selectionIndex+1 < viewer.getTable().getItemCount()) {
								selectionIndex++;
						...
						int selectionIndex = viewer.getTable().getSelectionIndex();
						if (selectionIndex+1 < viewer.getTable().getItemCount()) {
							selectionIndex++;
						}
						viewer.getTable().setSelection(selectionIndex);
						if (alwaysOpenEditor) {
	...
	/**
	 * ColumnLabelProvider that determines the styles to use in the internal editor control
	 * of the table viewer based on the registered styles in the ConfigRegistry for default
	 * style, conversion error style and validation error style.
	 * <p>
	 * If no explicit styles for conversion and validation errors are configured, the foreground
	 * is simply rendered in red.
	 */
	protected class InternalLabelProvider extends ColumnLabelProvider {
		 ...
		 * @param element The element for which the style should be searched
		...
		/**
		 * Applies style attributes to the internal cell editor of the table viewer
		 * based on the state of the value.
		 * @param editorControl The internal editor control of the table viewer to 
		 * 			set the style to
		 * @param element The element that is shown in that cell
		 */
		...
		public Font getFont(Object element) {
			return getActiveCellStyle(element).getAttributeValue(CellStyleAttributes.FONT);
		}

	}
	
	/**
	 * FocusListener that will fire the focus lost event only if the Text editor control of
	 * the editing support and the table viewer does not have the focus. This is because
	 * for NatTable use cases they share a focus. So closing the TableCellEditor should only
	 * happen if both loose focus. Using the default focus handling, opening an editor in
	 * the table via editing support would cause a focus lost event on the table, which
	 * immediately closes the editor again.
	 */
