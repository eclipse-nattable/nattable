 ...
 * by default, as a summation is not possible this way. 
 * You are able to change that behaviour by creating the SummationSummaryProvider
 * with the parameter strict set to <code>false</code>. In that case the non Number values
 * will be ignored.
 */
public class SummationSummaryProvider implements ISummaryProvider {
	...
	/**
	 * Create a new {@link SummationSummaryProvider} by using the given {@link IDataProvider}.
	 * <p>
	 * Using this constructor will set the {@link SummationSummaryProvider} in strict mode
	 * which means that if a column contains non Number values, {@link ISummaryProvider#DEFAULT_SUMMARY_VALUE}
	 * will be returned.
	 * @param dataProvider The {@link IDataProvider} that should be used to calculate the sum.
	 */
	 ...
	 * will be returned.
	 * @param dataProvider The {@link IDataProvider} that should be used to calculate the sum.
	 * @param strict If strict is set to <code>true</code> and one or more of the values in the
	 * 			column is not of type Number, then {@link ISummaryProvider#DEFAULT_SUMMARY_VALUE} 
	 * 			will be returned. If strict is set to <code>false</code>, this method will return 
	 * 			the sum of all the values in the column that are of type Number, ignoring the 
	 * 			non Number values.
	 */
